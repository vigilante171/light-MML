...
export const checkout = async (req, res) => {
  ...
  try {
    ...
    const [order] = await Order.create(
      [{ user: userId, items: orderItems, totalAmount, status: "PENDING", paymentStatus: "PENDING" }],
      { session }
    );
    ...
    for (const item of cart.items) {
      ...
      if (result.modifiedCount !== 1) {
        throw new Error(`Stock changed while processing "${item.product.name}". Please try again.`);
      }
    }
